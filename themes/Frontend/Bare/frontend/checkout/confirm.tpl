...
{block name='frontend_index_content'}
    ...
    {block name='frontend_checkout_confirm_form'}
        <form id="confirm--form" method="post" action="{if $sPayment.embediframe || $sPayment.action}{url action='payment'}{else}{url action='finish'}{/if}">
            ...
            {/block}
        {block name='frontend_checkout_cart_item_internalNumber'}
        <div id="frontend_checkout_cart_item_internalNumber" class="panel--actions">
        <label>Customer Order Number: </label><input type="text" name="internalNumber" value={$internalNumber}>
        </div>
        {/block}
        </form>
    {/block}
